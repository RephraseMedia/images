...
export default function Toolbar() {
  ...
  return (
    <div className="w-16 md:w-20 border-r border-border flex flex-col items-center py-4 gap-1 bg-background">
      {tools.map((tool) => (
        <button
          key={tool.id}
          onClick={() => setActiveTool(activeTool === tool.id ? null : tool.id)}
          className={`w-12 md:w-16 flex flex-col items-center gap-1 py-2 px-1 rounded-lg text-xs transition-colors ${
            activeTool === tool.id
        ...
        >
          {tool.icon}
          <span className="hidden md:block truncate w-full text-center">{tool.label}</span>
        </button>
      ))}
    </div>
  );
